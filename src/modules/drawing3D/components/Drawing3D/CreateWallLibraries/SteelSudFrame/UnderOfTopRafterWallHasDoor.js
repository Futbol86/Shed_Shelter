...
import {
    ...
    WALL_CLADDING_BOTH_SIDE, WALL_CLADDING_ONE_SIDE, WALL_CLADDING_NONE, 
    EXTERN_WALL_CLADDING_NONE, WALL_CLADDING_MATERIALS, 
    STEEL_STUD_INTERIOR_WALL_COLOR, STEEL_STUD_PLASTER_BOARD_COLOR, STEEL_STUD_EXTERN_PLASTER_BOARD_COLOR,
    EXTERN_WALL_CLADDINGS, FLOOR_HEIGHT,
...
var createWallHasDoorLocateHalfLeftAlongAxisX = async(params, scene) => {
    ...
    || (wallCladdingType === WALL_CLADDING_ONE_SIDE && (!externWallCladding || externWallCladding === EXTERN_WALL_CLADDING_NONE) )) {
        ...
        for(let i = 0; i < distanceBetweenDoors.length - 1; i++) {
            ...
            for(let j = 0; j < totalCrippleStud - 2; j++) {
                ...
                crippleStudIW.position.y += (STUD_WEB - WALL_STUD_WEB)/2;
        ...
        distanceBetweenDoors.map((data, idx) => {
            ...
            let newNogginIW = BABYLON.MeshBuilder.ExtrudeShape("noggin", {
                ...
                path: [
                    newV(0, 0, 0), newV(distance, 0, 0)
                ], 
                sideOrientation: BABYLON.Mesh.FRONTSIDE
            });

            newNogginIW.position.x = data[0] - STUD_FLANGE;
    ...
    for(let i = 0; i < distanceBetweenDoors.length - 1; i++) {
        ...
        headerDoorStudIW.position.x = distanceBetweenDoors[i][1] - STUD_FLANGE;
        headerDoorStudIW.position.z = doorHeight + STUD_FLANGE;
    ...
    || (wallCladdingType === WALL_CLADDING_ONE_SIDE) && (externWallCladding && externWallCladding !== EXTERN_WALL_CLADDING_NONE)) {
        ...
        for(let i = 0; i < distanceBetweenDoors.length - 1; i++) {
            ...
            let leftTrimmerStudIW = BABYLON.MeshBuilder.ExtrudeShape('leftTrimmerStud', {
                shape: [
                    newV(0, 0, 0), newV(-STUD_FLANGE, 0, 0), 
                    newV(-STUD_FLANGE, STUD_WEB, 0), newV(0, STUD_WEB, 0)  
                ], 
            ...
            let rightTrimmerStudIW = BABYLON.MeshBuilder.ExtrudeShape('rightTrimmerStud', {
                shape: [
                    newV(0, 0, 0), newV(STUD_FLANGE, 0, 0), 
                    newV(STUD_FLANGE, STUD_WEB, 0), newV(0, STUD_WEB, 0) 
                ], 
        ...
        || (wallCladdingType === WALL_CLADDING_ONE_SIDE && (!externWallCladding || externWallCladding === EXTERN_WALL_CLADDING_NONE) )) {
            ...
            distanceBetweenDoors.map((data, idx) => {
                ...
                wallComponentIndexes[wallName]["noggins"].push({
                    ...
                    "wallName": wallName,
                    "wallHeightType": wallHeightTypeName,
                    "length": floor(Math.round(distance * 10), -1),
                    "cutLength": Math.round(distance * 10), 
                    "quantity": 1
                });
            });
        }
...
var createWallHasDoorLocateHalfRightAlongAxisX = async(params, scene) => {
    ...
    for(let i = 0; i < distanceBetweenDoors.length - 1; i++) {
        ...
        let headerDoorStudIW = BABYLON.MeshBuilder.ExtrudeShape("headerDoor", {
            shape: [
                newV(STUD_FLANGE, STUD_WEB, 0), newV(0, STUD_WEB, 0), 
                newV(0, 0, 0), newV(STUD_FLANGE, 0, 0)
            ], 
