...
class DrawPartitionWall extends React.Component {
    ...
    render(){
        const {dashLines, verticleArrows, horizontalArrows, boldLines, normalLines, slopes, allDoors, allMullions, allBraces, scale, isHidden, isQuotePrint, title, grid}
            = this.props;
        return (
            <React.Fragment>
                <Stage height={this.props.stageHeight} width={this.props.stageWidth}
                       style={{display: (isHidden ? 'none' : '')}}
                       ref={ref => this.props.setStageRef(`internalWall${grid}`, ref)}
                >
                    <Layer>
...
export default DrawPartitionWall;