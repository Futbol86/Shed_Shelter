...
import {reduxForm, formValueSelector, change, getFormValues, SubmissionError} from "redux-form";
...
import {OR_SDE_loadUserList, OR_SDE_setActiveStaff, douploadInsurancePolicyFile, doDeleteAnInsurancePolicyFile, doClearInsurancePolicyFile, clearAContructionDataEntryInfo} from '../actions';
import {DOC_changeActiveModal, DOC_changeTypeModal} from "../../documents/actions";

import {getOrSdeUserList, getInsurancePolicyFile} from "../selectors";
...
class ContructionDataEntryAdd extends Component {
    ...
    componentDidUpdate(prevProps) {
        const {userId, contructionDataFormData} = this.props;
        if(isEmpty(contructionDataFormData)) {
            let initialCDEForm = {
                createdBy: userId, 
                insurancePolicyType: 'Text'
            }
            
            this.props.initialize(initialCDEForm);
        }
    ...
    handleDeleteInsuranceFile = (id) => {
        this.props.openModalAction({
            ...
            onConfirm: () => this.props.doDeleteAnInsurancePolicyFile({id})
        });
    };
    ...
    render() {
        const {contructionDataFormData} = this.props;
        const uploadRootURL = process.env.REACT_APP_STATIC_FILE_URL2;
        let insurancePolicyType = contructionDataFormData && contructionDataFormData.insurancePolicyType;
...
const mapStateToProps = (state) => ({
    ...
    contructionDataFormData:      getFormValues(CONTRUCTION_DATA_ENTRY_DETAIL_FORM_NAME)(state),
