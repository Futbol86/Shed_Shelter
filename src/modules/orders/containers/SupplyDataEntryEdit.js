...
import {loadASupplyDataEntryInfo, OR_SDE_loadUserList, OR_SDE_setActiveStaff, clearASupplyDataEntryInfo} from '../actions';
import {DOC_changeActiveModal} from "../../documents/actions";

import {getOrSdeUserList, getSupplyDataEntryInfo} from "../selectors";
...
class SupplyDataEntryAdd extends Component {
    componentDidMount() {
        let {supplyDataEntryId} = this.props.match.params;
        if (supplyDataEntryId) {
            this.props.loadASupplyDataEntryInfo({id: supplyDataEntryId});
        }
    ...
    componentDidUpdate(prevProps) {
        const {savedSupplyDataEntryDetail, userList} = this.props;
        const {supplyDataEntryDetail} = prevProps;

        if ((!supplyDataEntryDetail || isEmpty(supplyDataEntryDetail))
            && !isEmpty(savedSupplyDataEntryDetail) && userList && userList.length
        ) {
            let initialSDEForm = {
                ...savedSupplyDataEntryDetail,
                supplyType: savedSupplyDataEntryDetail.supplyType + '',
                userId: savedSupplyDataEntryDetail.userId + ''
            }
...
const mapStateToProps = (state) => ({
    ...
    supplyDataEntryDetail:      getFormValues(SUPPLY_DATA_ENTRY_DETAIL_FORM_NAME)(state),

    savedSupplyDataEntryDetail: getSupplyDataEntryInfo(state),
});

const mapDispatchToProps = (dispatch) => ({
    loadASupplyDataEntryInfo:   payload => dispatch(loadASupplyDataEntryInfo(payload)),
