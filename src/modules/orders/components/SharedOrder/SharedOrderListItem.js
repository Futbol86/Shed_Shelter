...
const SharedOrder = ({ order }) => {
    const { id, quoteDetails, status, rollFormDetails, supplierDetails, createdAt, updatedAt} = order; 

    return (
        <tr>
            <td>
                <NavLink to={`/orders/shared-orders/edit/${quoteDetails.id}`} title="Edit Order" className="p-1">
                    {quoteDetails.id}
                </NavLink>
            </td>
            <td>
